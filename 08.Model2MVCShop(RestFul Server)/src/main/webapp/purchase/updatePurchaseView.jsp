...
<script type="text/javascript">
	...
	function checkPhone() {
		let receiverPhone = document.querySelector('input[name="receiverPhone"]').value;
		
		if(receiverPhone.length === 13) {
			const parts = receiverPhone.split('-');
			if( parts.length === 3 ) {  
				for (const part of parts) { 
					if ( isNaN(Number(part)) ) {  
						alert("올바른 휴대폰 전화번호를 입력해주세요. (형식 : 010-0000-0000)");
						return false;
					}  
				}  
				return true;
	...
	function fncUpdatePurchase() {
		if (checkNull() === true && checkPhone() === true) {
