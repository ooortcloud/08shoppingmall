...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
	...
	<c:if test="${ resultPage.currentPage > resultPage.pageUnit }">
			<a href="javascript:fncGet<c:if test="${requestScope.title == 'user' }">User</c:if><c:if test="${requestScope.title == 'product' }">Product</c:if><c:if test="${requestScope.title == 'purchase' }">Purchase</c:if>List('${ resultPage.currentPage-1}')">◀ 이전</a>
	</c:if>
